...
class MGRegisterPage extends StatefulWidget {
  /// 倒计时的秒数，默认60秒。
  final int countdown = 60;
  OnCodeLoginPageListener? onCodeLoginPageListener;

  MGRegisterPage({Key? key, this.onCodeLoginPageListener}) : super(key: key);

  @override
  _MGRegisterPageState createState() => _MGRegisterPageState();
}

class _MGRegisterPageState extends State<MGRegisterPage> {
  /// 倒计时的计时器。
  late Timer _timer;
                          ...
                          new Container(
                              ...
                              child: new RaisedButton(
                                onPressed: () {
                                  ...
                                  // _fireRegisterNet("15717914505", "843679", "abc12345");
                                },
                                shape: StadiumBorder(),
  ...
  Future<Null> _fireSmsNet(String telStr) async {
    if (!YLZValidatedExtent.MobileIsValidated(telStr)) {
      ...
      var result = await MGLoginDao.registerSmsCode(telStr);
      if (result["code"] != 1) {
  ...
  Future<Null> _fireRegisterNet(
    ...
    if (!YLZValidatedExtent.MobileIsValidated(telStr)) {
      ...
      var result = await MGLoginDao.register(
